...
func (c *BikeCalc) SetWeightWheels(x float64) {
	...
	// aproximated 0.9 reduction of the wheel radius, because
	// the mass is not rotating at the outer edge of the wheel.
	c.massKin = c.mass + c.rotatingMass*0.9
...
func (c *BikeCalc) Fdrag(v float64) float64 {
	return c.cDrag * signedSquare(v+c.wind)
}
