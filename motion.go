...
// deriv > 0 => too long left step is not possible for power > 0, proof?.
...
// and air drag coeffient cDrag clearly > 0. Iteration is stopped when change in speed
// is less than tol. This gives error < tol^2 x 0.1. v is initial speed for iteration.
func (c *BikeCalc) NewtonRaphson(power, tol, v float64) (vel float64, iter int) {
	const bias = 0.08
	//bias is used to shifts NR roots to the left by distance Δv*Δv*bias.
	...
	if c.errmsg == nil {
		c.appendErr(": Newton-Raphson did not converge (100 iterations)")
	}
	return v, 0
}
...
// based on Standard Atmosphere sea level air pressure.
...
func (c *BikeCalc) RhoFromEle(ele float64) (rho, temperature float64) {
	...
	y := -(1.0 + g*M/(R*L))	
	z := y * x * (1 - x*0.5)
	rho *= 1 + z*(1+z*(0.5+z*(1.0/6.0))) //rho *= math.Pow(1+x, y)
